...
public class YongzhouIC extends CityCompatible {
	...
	public boolean excutCmdResponse(String arg0, String arg1, byte[] arg2) {
		...
		} else if ("card_date".equals(arg0)){
			String resp = ByteUtils.getHex(arg2);
			Log.e("Query_Card_Date","Query Date Result=" +  resp);
			mDate = getCardDate(resp);
		}
		else if("card_enabled".equals(arg0)){
			String resp = ByteUtils.getHex(arg2);
			mEnabled = getCardState(resp);
			Log.e("Query_Card_Enabled","Query Enabled Result=" + resp);
		}
			...
			if (arg0 != null && arg0.length > 0 && initial()) {
				for (int i = 0; i < arg0.length; i++) {
					...
					case CARD_BLANCE:
						...
						break;
					case CARD_DATE:
						mXmlCmdKey.add("card_date");
						NFCCardManager.excuXmlCmd();
						budle.putString(CARD_DATE,mDate);
						break;
					...
					case CARD_CHARGE_RECODE:
						budle.putParcelableArrayList(CARD_CHARGE_RECODE,
								getmChargeRecode());
						break;
					case "card_enabled"://查询卡片是否启用
						mXmlCmdKey.add("card_enabled");
						NFCCardManager.excuXmlCmd();
						budle.putBoolean("card_enabled",mEnabled);
						break;
					}
				}
			}
...
/*	*//**
	...
	private  String getCardDate(String resp){
		String cardDate="";
		if(resp.endsWith("9000"))
			cardDate = resp.substring(0,resp.length()-4);
		return  cardDate;
	}
	private  boolean getCardState(String resp){
		String cardEnabled="";
		if(resp.endsWith("9000"))
			cardEnabled = resp.substring(0,resp.length()-4);
		return  cardEnabled.equals("01");
	}
	  /**
     * 二次格式解析后余额
     * @return
